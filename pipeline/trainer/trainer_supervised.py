...
class TrainerSupervised:
    ...
    def train_step(self, input_data: torch.Tensor, noisy_ref: torch.Tensor, ref:torch.Tensor):
        ...
        loss,loglike_loss,perc_loss = self.loss(model_output, noisy_ref, ref)
        loss.backward()

        self.optimizer.step(closure=None)
        
#         return loss.cpu().data.numpy(),0,perc_loss.cpu().data.numpy()

        return loss.cpu().data.numpy(),loglike_loss.cpu().data.numpy(),perc_loss.cpu().data.numpy()
    ...
    def log_train_step(self, epoch_id: int, step_id: int, epoch_time: float, loss: float, perc_loss: float, loglike_loss: float):
        if self.print_frequency is None or step_id % self.print_frequency == 0:
            LOGGER.info("[{} s] Epoch {}. Train step {}. Loss {}. Loss log likelihood {}. Loss perceptual {}".format(
                epoch_time, epoch_id, step_id, loss,loglike_loss,perc_loss))
            return True
    ...
    def run_train_epoch(self, epoch_id: int):
        ...
        for step_id, (input_data, noisy_ref, ref) in enumerate(self.train_data_loader):
            loss,loglike_loss,perc_loss = self.train_step(input_data, noisy_ref, ref)
            epoch_time = time.time() - start_time

            mean_loss += loss
            step_count += 1

            self.log_train_step(epoch_id, step_id, epoch_time, loss,perc_loss,loglike_loss)
