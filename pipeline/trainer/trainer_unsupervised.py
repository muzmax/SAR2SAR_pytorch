...
#             print('mean:{}, max:{}, min{}'.format(torch.mean(grad_abs),torch.max(grad_abs),torch.min(grad_abs)))
    ...
    def predict_step(self ,input_data: torch.Tensor ,name: str ,epoch_id, step_id ,denorm):
        ...
        save_im(tresh_im(result,treshold=save_tresh),'./data/sample/{}_{}_epoch_{}.png'.format(name[0],self.phase,epoch_id))
        np.save('./data/sample/{}_{}_epoch_{}.npy'.format(name[0],self.phase,epoch_id),result)
        


    def log_train_step(self, epoch_id: int, step_id: int, epoch_time: float, loss: float):
        if self.print_frequency is None or step_id % self.print_frequency == 0:
            LOGGER.info("[{} s] Epoch {}. Train step {}. Loss {}".format(
                epoch_time, epoch_id, step_id, loss))
            return True
    ...
    def run_train_epoch(self, epoch_id: int):
        ...
        for step_id, (input_data, target) in enumerate(self.train_data_loader):
            ...
            self.log_train_step(epoch_id, step_id, epoch_time, loss)
